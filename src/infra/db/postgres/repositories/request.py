...
class RequestRepository:
    ...
    async def create(self, request: RequestEntity)-> RequestEntity:
        ...
        return RequestEntity(id=request_db.id, user_id=request_db.user_id, input_text=request_db.input_text, response_text=request_db.response_text)
    ...
    async def get_requests_by_user_id(self, user_id: int)-> List[RequestEntity]:
        ...
        print(requests_db)
        return [RequestEntity(id=request.id, user_id=request.user_id, input_text=request.input_text, response_text=request.response_text) for request in requests_db]

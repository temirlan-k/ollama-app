...
class DIContainer(containers.DeclarativeContainer):
    ...
    analytics_service = providers.Factory(AnalyticsService,analytics_repo=analytics_repo, logger=logger)
    user_service = providers.Factory(UserService, uow=uow,logger=logger)
    request_service = providers.Factory(RequestService, uow=uow, llm=llm, analytics_service=analytics_service,logger=logger)

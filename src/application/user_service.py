...
class UserService:
    ...
    def __init__(self, uow: SQLAlchemyUnitOfWork, logger: structlog.stdlib.BoundLogger) -> None:
        self._uow = uow
        self._logger = logger
    ...
    async def create_user(self, user: UserRequestDTO) -> dict:
        ...
        async with self._uow as uow:
            try:
                ...
                if existing_user:
                    self._logger.warning("User already exists", extra={"email": user.email})
                ...
                self._logger.info("User created successfully", extra={"email": user.email, "user_id": created_user.id})
                return {"access_token": create_access_token({"sub": created_user.id, "token_type": "bearer"})}
            except Exception as e:
                await uow.rollback()
                self._logger.error("Error creating user", extra={"email": user.email, "error": str(e)})
                raise e
    ...
    async def get_user_by_email(self, email: str) -> UserEntity | None:
        ...
        async with self._uow as uow:
            try:
                ...
                self._logger.error("Error fetching user", extra={"email": email, "error": str(e)})
                raise e
        ...
        async with self._uow as uow:
            try:
                ...
                if not PasswordHandler.verify(exist_user.password, user.password):
                    self._logger.warning("Invalid password", extra={"email": user.email})
                    raise BadRequestException('Invalid password or email')

                self._logger.info("User logged in successfully", extra={"email": user.email, "user_id": exist_user.id})
                return {"access_token": create_access_token({"sub": exist_user.id, "token_type": "bearer"})}
            except Exception as e:
                self._logger.error("Error logging in", extra={"email": user.email, "error": str(e)})
                raise e
